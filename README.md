...
Методы реализуют загрузку товаров и последующее изменения списка.
```
```
Класс OrderData
Функция: Формирование и отправка заказа.
Поля:
cards: тип: string[] - массив товаров в заказе;
...
Методы должны реализовывать проверку введенных данных и отправку заказов.
```
```
Класс CartPopup 
...
Поля:
cards: ICard[] - массив товаров, которые добавили в корзину.
Методы позволяют добавлять и удалять товары из корзины, а также выводить их количество.
```
### View (Представление) 
...
Конструктор принимает контейнер для вывода товаров и объект для
обработки событий.

Поля:
container: HTMLElement - основной контейнер страницы;
container-Gallery: HTMLElement - контейнер для товаров;
counterCart: HTMLElement - счетчик товаров в корзине;
buttonCart: HTMLElement - кнопка корзины.
Методы: 
...
addButton: HTMLElement - кнопка КУПИТЬ;
deleteButton: HTMLElement - кнопка УДАЛИТЬ.
Методы:
...
closeButton: HTMLButtonElement — кнопка ЗАКРЫТЬ;
